...
updateBullets() {
  ...
  for ( b = stage.bulletHead.next; b != NULL; b = b->next ) {
    ...
    if ( bulletHitFighter( b ) || b->x < -b->w || b->y < -b->h || b->x > SCREEN_WIDTH ||
         b->y > SCREEN_HEIGHT ) {
...
fireAlienBullet( Entity *e ) {
  ...
  calcSlope( player->x + ( player->w / 2 ), player->y + ( player->h / 2 ), e->x, e->y, &bullet->dx,
             &bullet->dy );
 ...
 */
static void
addExplosion( int x, int y, int num ) {
  Explosion *e;
 ...
 */
static void
addExplosionParticle( int x, int y, int num ) {
  ExplosionParticle *exp;
